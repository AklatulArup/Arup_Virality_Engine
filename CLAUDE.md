...
- `src/lib/early-share.ts` — per-creator build-up signal from the sibling cross-section (interim stand-in for decay-fit on YouTube/Shorts until velocity data matures). `estimateEarlyShare(creatorHistory, platform, nowMs, defaultShare)` compares median views of siblings aged 4–21d vs 21d+; the observed ratio's position between what the evergreen and front-loaded curves would each predict (self-calibrated from the actual sibling ages) becomes a 0–1 weight that interpolates the default curve toward a front-loaded one (~50% by d2, 80% by d7, 95% by d30). Null when either bucket has <3 siblings → default curve, zero regression. Threaded as `ForecastInput.earlyShareSignal`; curve priority inside `composeShareAt()`: fitted decay > early-share blend > hand-tuned. Pure compute, no network. Born from the 2026-06-10 blind backtest where trading channels' day-7 actuals beat the evergreen curve 5–30×.
